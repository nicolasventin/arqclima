...
**Pendiente para la Etapa 8 (Trabajos)**: evaluar si el alcance de Andrés en `apps.stock` (hoy `manage_stock_general`, sin acotar) conviene restringirlo a "solo movimientos de stock relacionados con sus propios trabajos asignados" — ver decisión 30 más abajo. También es el momento de agregarle a `MovimientoStock` una FK real y nullable (`trabajo`, y `orden_compra` si corresponde) con una migración normal, reemplazando/complementando `referencia_libre` — ver decisión 32 más abajo (se descartó `GenericForeignKey` a propósito).
